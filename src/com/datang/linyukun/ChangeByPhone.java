...
public class ChangeByPhone {
	...
	public static void changeAdPwd() throws Exception {
		System.out.println("请输入您要更改密码的账号:");
		Scanner input = new Scanner(System.in);
		String number = input.nextLine().trim();
		System.out.println("请输入您当时注册用手机号:");
		...
		while (re != 0) {
			...
			while (a >= 0) {
				if (a == 0) {
					System.err.println("您输入的错误已达上限正在返回上一级!");
					ChangeAdPwd.changAdPwd();
				}
				a = --i;
				if (a != 0) {
					System.err.println("您输入的账号或手机号有误!,当前剩余尝试次数:" + a);
					System.err.println("是否继续?请输入(y or n):");
